...
func NewCodedLinksRepository(cfg *config.Config) (*CodedLinksRepository, error) {
	...
	repository := &CodedLinksRepository{
...
func (lr *CodedLinksRepository) LoadFromFile() error {
	lr.rwMutex.Lock()
...
func (lr *CodedLinksRepository) Close() error {
	lr.rwMutex.Lock()
	defer lr.rwMutex.Unlock()
